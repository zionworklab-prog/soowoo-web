...
- Notion의 카테고리 값에 "소주"/"소츄"가 포함되면(예: 고구마소츄, 보리소츄) 사이트에서는 하나의 **소츄** 필터로 합쳐집니다.
- "사케" / "전통주" 문자열을 포함하는 카테고리만 노출됩니다. 잔음료, 리큐어 등 다른 카테고리는 자동으로 제외됩니다.
- 세부 구분(고구마/보리, 잔/병 등)은 `종류` 필드로 상세 팝업에 표시됩니다.

## 메뉴 화면 구조

- `/menu`는 전체/사케/소츄/전통주 필터 버튼과 함께 술을 큰 썸네일의 그리드(2~3열)로 보여줍니다.
- 항목을 클릭하면 페이지 이동 없이 같은 화면에 팝업(모달)으로 상세 정보(도수, 산도, 지역, 주도, 정미보합, 테이스팅노트, 페어링 등)가 뜹니다. 온라인 주문 사이트처럼 보이지 않도록 의도적으로 페이지 전환 없이 팝업으로만 구현했습니다.
